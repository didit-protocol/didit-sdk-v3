...
export class UiTabs extends LitElement {
  ...
  @property() public localTabHeight = '39px'
  ...
  public override render() {
    ...
    this.style.cssText = `
      ...
      --local-tab-height: ${this.localTabHeight};
    `
    ...
    return this.tabs.map((tab, index) => {
      ...
      return html`
        ...
        >
          <ui-text variant="tab-link" color="inherit"> ${tab.label} </ui-text>
        </button>
      `
    })
  }
  ...
  private animateTabs(index: number, initialAnimation: boolean) {
    ...
    const passiveBtnText = passiveBtn?.querySelector('ui-text')
    const activeBtnText = activeBtn?.querySelector('ui-text')
    ...
    if (activeBtn && activeBtnBounds && activeBtnTextBounds && activeBtnText) {
      if (index !== this.activeTab || initialAnimation) {
        ...
        this.localTabHeight = `${
          Math.round(activeBtnBounds.height + activeBtnTextBounds.height) + 6
        }px`
